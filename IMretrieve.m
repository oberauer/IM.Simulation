...
if ismember(E.test, 1:3)
    ...
    if E.PreRetro == 1 || E.CTI(cueing) == 0 || refocus == 1  % if retrieval has not yet happened in the CTI from the same location ...
    ...
    if E.test == 2  % old-new recognition: after deciding on which feature is retrieved, now decide on which response to give
        ...
        probe = find(C.stim(probeIdx,:) == max(C.stim(probeIdx, :)), 1);
        SameRange = circshift(E.SameRange, [1, probe-180]); % E.SameRange is centered on pi
        ChangeRange = 1-SameRange;
...
if E.outsize > 1   % only if further tests follow
    % output interference, decay and erasure of FX
    W = W * (1-P.outputinterference);
    [Map, W] = IMdecayFX(Map, W, rt);
    Map = UpdateFX(Map); % response --> erase FX
end
