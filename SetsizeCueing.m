...
% size (1 to 8) and cueing condition (neutral, valid, and invalid)
...
Mdevobs = NaN(E.nsubj, 3, E.maxsetsize);  % mean observed deviation of responses from target feature for subjects, cueing conditions, and set size
Mrt = NaN(E.nsubj, 3, E.maxsetsize);      % mean RT
MMSD = NaN(E.nsubj, 3, E.maxsetsize);     % Mixture Model SD parameter
    ...
    for setsize = 1:E.maxsetsize
        ...
        for cueing = 1:3  % neutral, valid, invalid
            ...
            if fitMM
                ...
                disp('    id        setsize  cueing   iteration  Deviance SD(deg)   P(guess)  P(trans)   P(CW)');
                disp([id, setsize, cueing, itercount, MMloglik/1000, MMparms]);
                MMSD(id, cueing, setsize) = MMparms(1);
                MMguessing(id, cueing, setsize) = MMparms(2);
                if npar > 2, MMtranspos(id, cueing, setsize) = MMparms(3); end
...
plot(plotvector);
PostFigure([0.8,E.maxsetsize+1, 0, 80], 'Setsize', 'Deviation (Deg)', [], legendtext);
...
plotvector = squeeze(mean(Mdevobs,1))';
plot(plotvector);
PostFigure([0.8,E.maxsetsize+1, 0, 1.05*max(max(plotvector))], 'Setsize', 'Deviation', 'Mean Deviation', legendtext);
subplot(2,2,2);
Mrt(:,3,1) = NaN;
...
if fitMM
    ...
    PostFigure([0.8,E.maxsetsize+1, 0, max(max(mean(MMSD,1)))+0.5], 'Setsize', 'Mean SD', 'SD from Bays Mixture', {'Neutral', 'Valid', 'Invalid'});
    subplot(3,2,2);
    plot(meanK');
    PostFigure([0.8,E.maxsetsize+1, 0, 6], 'Setsize', 'K', 'K from Bays Mixture', {'Neutral', 'Valid', 'Invalid'});
    subplot(3,2,3);
