...
if singleDual == 2
    ...
    drate = ones(nsteps, 1);
    A = cumsum(drate * Evidence + randn(nsteps, dSetSize)*P.dnoise);  % outer product of drate and Adrift -> matrix of tstep rows and 360 columns, each row = addition to to the 360 accumulators
    ...
    decisionresponse = find(A(t,:)==max(A(t,:)), 1);
    rt = t.*C.tstep + overTime;
    correct = decisionresponse == selectedStim;
    [Map, W] = IMdecayFX(Map, W, rt); 
else
    decisionresponse = NaN;
    rt = NaN;
    correct = NaN;
end
...
if E.test == 1 || E.test == 4, memresponse = zeros(1, E.outsize); end
...
output.response = memresponse;
output.rt = memrt;
output.dresponse = decisionresponse;
output.dcorrect = correct;
output.drt = rt;
