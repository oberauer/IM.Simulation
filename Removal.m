...
fitIM = 0;
Cueval = [0.5, 0.6, 0.7, 0.8, 0.9, 0.98]; 
...
Mdevobs = NaN(E.nsubj, 3, length(Cueval));  % mean observed deviation of responses from target feature for subjects, cueing conditions, and set size
Mrt = NaN(E.nsubj, 3, length(Cueval));      % mean RT
MMSD = NaN(E.nsubj, 3, length(Cueval));     % Mixture Model SD parameter
MMguessing = NaN(E.nsubj, 3, length(Cueval));  % Mixture Model Guessing parameter
MMtranspos = NaN(E.nsubj, 3, length(Cueval));  % Mixture Model Transposition parameter (swap error proportion)
MMcwattraction = NaN(E.nsubj, 3, length(Cueval));  % Mixture Model colorwheel-attraction strength parameter
Mwact = NaN(E.nsubj, 3, length(Cueval));       % mean activation in the binding weight matrix
...
% responses
Array = zeros(E.nsubj*3*length(Cueval)*E.ntrials, setsize);
Target = zeros(1,E.nsubj*3*length(Cueval)*E.ntrials);
Response = zeros(1,E.nsubj*3*length(Cueval)*E.ntrials);
...
for id = 1:E.nsubj
    ...
    for cval = 1:length(Cueval)
        
        E.cuevalidity = Cueval(cval);
        ...
        for cueing = 1:3  % neutral, valid, invalid
            ...
            Mdevobs(id, cueing, cval) = mean(abs(fdistance));  %mean deviation (averaged over trials)
            Mrt(id, cueing, cval) = mean(rt);
            Mwact(id, cueing, cval) = mean(wact);
            ...
            if fitMM
                ...
                disp([id, Cueval(cval), itercount, MMloglik]);
                MMSD(id, cueing, cval) = MMparms(1);
                MMguessing(id, cueing, cval) = MMparms(2);
                if npar > 2, MMtranspos(id, cueing, cval) = MMparms(3); end
                if npar > 3, MMcwattraction(id, cueing, cval) = MMparms(4); end
            end
...
if fitMM
    ...
    PreFigure;
    subplot(3,2,1);
    plot(Cueval, squeeze(mean(MMSD,1))');
    PostFigure([min(Cueval)-0.1,1, 0, max(max(mean(MMSD,1)))+0.5], 'Setsize', 'Mean SD', 'SD from Bays Mixture', {'Neutral', 'Valid', 'Invalid'});
    subplot(3,2,2);
    plot(Cueval, meanK');
    PostFigure([min(Cueval)-0.1,1, 0, 6], 'Setsize', 'K', 'K from Bays Mixture', {'Neutral', 'Valid', 'Invalid'});
    subplot(3,2,3);
    plotCueval, (squeeze(mean(MMPm,1))');
    PostFigure([min(Cueval)-0.1,1, 0, 1], 'Setsize', 'Mean P(m)', 'P(mem)');
    subplot(3,2,4);
    plot(Cueval, squeeze(mean(MMguessing,1))');
    PostFigure([min(Cueval)-0.1,1, 0, 0.5], 'Setsize', 'Mean P(guess)', 'P(guess)');
    subplot(3,2,5);
    plot(Cueval, squeeze(mean(MMtranspos,1))');
    PostFigure([min(Cueval)-0.1,1, 0, 1], 'Setsize', 'Mean P(trans)', 'P(transpos)');
    subplot(3,2,6);
...
%%% Save results
if E.saveResults == 1
    fid = fopen(['IMSim.Removal', mat2str(setsize), '.dat'], 'w');
    for id = 1:E.nsubj
        for cval = 1:length(Cueval)
            for cueing = 1:3
                fprintf(fid, '%d %d %d %d  ', id, cval, cueing, Mdevobs(id, cueing, cval));
                if (fitMM == 1), fprintf(fid, '%d %d %d %d', MMtranspos(id, cueing, cval), MMguessing(id, cueing, cval), MMcwattraction(id, cueing, cval), MMSD(id, cueing, cval)); end
                fprintf(fid, '\n');
            end
        end
    end
