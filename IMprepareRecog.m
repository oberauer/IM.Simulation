...
likSame = C.ContentFun(C.x, pi, P.kappacrit*P.kappa_feat);  % estimated likelihood of "same" trials using the meta-cognitive estimate of feature precision
...
if expectedChange == 0  % all stimuli equally likely as change probes
    ...
    likChange = mean(C.ContentFun(C.x, Lures, P.kappacrit*P.kappa_feat));
    E.SameRange = likSame > likChange; 
else
    likChange1 = C.ContentFun(C.x, pi+deg2rad(expectedChange), P.kappacrit*P.kappa_feat); % expect large changes: mean 90 degrees
    likChange2 = C.ContentFun(C.x, pi-deg2rad(expectedChange), P.kappacrit*P.kappa_feat); % expect large changes: mean 90 degrees
